...
public class CheckoutTest extends BaseTest {
	...
	// Finish Button → Confirmation Page
	@Description("Clicking Finish completes the order and redirects to confirmation")
	@Severity(SeverityLevel.CRITICAL)
	@Test(groups = { "Sanity", "Checkout" })
	public void finishButton_placesOrderSuccessfully() {
		getDriver().manage().deleteAllCookies();
		getDriver().get(ConfigReader.getProperty("baseUrl"));
		new LoginPage(getDriver()).login("standard_user", "secret_sauce");
		...
		CheckoutPage checkout = new CheckoutPage(getDriver());
		checkout.fillCheckoutForm("Kalai", "Vani", "12345");
		checkout.clickContinue();

		Assert.assertTrue(checkout.isOnOverviewPage(), "Overview page not loaded");
		checkout.clickFinish();

		Assert.assertTrue(getDriver().getCurrentUrl().contains("checkout-complete.html"),
				"Did not reach confirmation page");

		test.pass("✅ Finish button successfully placed the order and navigated to confirmation.");

	}

	// Cancel Button → Back to Inventory
	@Description("Cancel button should navigate back to inventory from overview")
	@Severity(SeverityLevel.NORMAL)
	@Test(groups = { "Navigation", "Checkout" })
	public void cancelButton_navigatesToInventory() {
		getDriver().get(ConfigReader.getProperty("baseUrl"));
		new LoginPage(getDriver()).login("standard_user", "secret_sauce");

		InventoryPage inventory = new InventoryPage(getDriver());
		inventory.addSpecificProductToCart("Sauce Labs Backpack");
		...
		Assert.assertTrue(checkout.isOnOverviewPage(), "Not on overview page");

		checkout.clickCancel();

		Assert.assertTrue(getDriver().getCurrentUrl().contains("inventory.html"), "Cancel did not return to inventory");

		test.pass("✅ Cancel button returned user to inventory as expected.");
	}
